//link:https://leetcode.com/contest/biweekly-contest-113/problems/minimum-edge-reversals-so-every-node-is-reachable/
...
public:
    ...
    vector<int>depth;
    int tc=0;
    void dfs(int node, int dep, vector<pair<int,int>>adj[], vector<int>&vis){
        vis[node]=1;
        depth[node]=dep;
        int c=0;
        for(auto it:adj[node]){   
            ...
            if(!vis[newnode]){
               ...
               tc+=weight; 
               cost[newnode]=cost[node]+weight; 
               dfs(newnode,dep+1,adj,vis);
            }
        }
    }
