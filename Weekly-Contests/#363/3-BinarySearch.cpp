...
public:
    ...
    bool isPossibleToMakeKAlloys(long long k, int budget, vector<int>&comp, vector<int>&stock, vector<int>&costt){
        int n=comp.size();
        long long cost=0;
        for(int i=0;i<n;++i){
            cost+=max(0ll,( ((k*comp[i])*1ll-stock[i]*1ll)*costt[i]*1ll) );
        }
        return cost<=(budget*1ll);
    }
    int maxNumberOfAlloys(int n, int k, int budget, vector<vector<int>>& composition, vector<int>& stock, vector<int>& cost) {
        int res=0;
        for(int i=0;i<k;++i){
            vector<int>comp=composition[i];
            int l=0, r=1e9;
            while(l<(r-1)){
                int mid=(l+r)>>1;
                if(isPossibleToMakeKAlloys(mid*1ll,budget,comp,stock,cost)){
                    l=mid;
                } else {
                    r=mid;
                }
