//great question on bit manipulation
//link:https://leetcode.com/contest/weekly-contest-360/problems/minimum-operations-to-form-subsequence-with-target-sum/
//aryan's video:https://www.youtube.com/watch?v=eVrB_4K5wcs
...
public:
    int minOperations(vector<int>& nums, int target) {
        int n=nums.size();
        vector<int>present(31,0);
        long long sum=0;
        fl(i,n){
            present[(int)(log2(nums[i]))]++; 
            sum+=(nums[i]*1ll);
        }
        if(sum<target){
            return -1;
